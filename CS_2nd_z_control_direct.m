...
% Parameters
d = 150; %2;           % agent dimension
N = 150; %10;          % number of agents
lam = 10;
ht = 0.01;
t0 = 0;
tol_consensus = 1e-15;
        ...
        for j = 1:N
            if j ~= i
                ...
                sum_interaction = sum_interaction + ...
                    phi(dist) * (vnew{j}(:,k-1) - vnew{i}(:,k-1));
            end
        end
        u{i}(:,k-1) = -lam * (vnew{i}(:,k-1) - v_mean(:,k-1));
        xnew{i}(:,k) = xnew{i}(:,k-1) + ht * vnew{i}(:,k-1);
        vnew{i}(:,k) = vnew{i}(:,k-1) + ht * (sum_interaction/N + u{i}(:,k-1));
        X_k(:,i) = xnew{i}(:,k);
        V_k(:,i) = vnew{i}(:,k);
    end
    x_mean(:,k) = mean(X_k,2);
